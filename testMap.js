...
const fetchFeaturedDeals = async (type = 'latest', city = '') => {
    try {
...
const fetchFeaturedProjects = async (status = '', city = '') => {
    // Cache check
    const now = Date.now();
...
const fetchAvailableUnits = async (project, block) => {
    try {
...
; async function run() { const deal = (await axios.get('http://localhost:4000/api/public/listings/sector-4-kurukshetra-540', { headers: { 'X-API-KEY': 'BP-WEB-INTEGRATION-2026-X7Y9' } })).data.data; try { const mapped = mapDealToProperty(deal); console.log('Mapped successfully'); } catch(e) { console.error('Map error:', e); } } run();